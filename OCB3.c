...
static void calc_L_i(block l, block ldollar, unsigned i)
{
    ...
    for ( ; (i&1)==0 ; i>>=1)
        double_block(l,l);            /* podwajanie dka ka¿dnych koñcowych 0 */
}


static void hash(block result, unsigned char *k, unsigned char *a, unsigned abytes)
{
    AES_KEY aes_key;
    block lstar, ldollar, offset, sum, tmp;
    unsigned i;
    
    /* Zmienne zale¿ne od klucza */
    ...
    /* Przetwarzanie ka¿dego pe³nego bloku */
    ...
    /* Przetwarzanie ka¿dego czêœciowego, koñcowego bloku; obliczanie koñcowej wartoœci hasha */
...
static int ocb_crypt(unsigned char *out, unsigned char *k, unsigned char *n, unsigned char *a, unsigned abytes, unsigned char *in, unsigned inbytes, int encrypting)
{
    ...
    /* Ustawianie AES'a i œci¹ganie z szyfrogamu taga uwierzytelniaj¹cego */
    if ( ! encrypting )
	{
         if (inbytes < TAGBYTES)
			 return -1;
    ...
    /* Zmienne zale¿ne do nonce i przygotowywane do szyfrowania */
    ...
    /* Hashowanie skojarzonych danych */
    ...
    /* Przetwarzanie ka¿dego czêœciowego, koñcowego bloku i obliczanie surowego taga uwierzytalniej¹cego */

    inbytes = inbytes % 16;  /* Bajty w ostatnim bloku */
    if (inbytes > 0)
	{
        /* Offset_* = Offset_m xor L_* */
        xor_block(offset, offset, lstar);
    ...
    if (encrypting)
	{
        ...
        return (memcmp(in,tag,TAGBYTES) ? -1 : 0);     /* Sprawdzanie poprawnoœci */
}

#define OCB_ENCRYPT 1
#define OCB_DECRYPT 0

void ocb_encrypt(unsigned char *c, unsigned char *k, unsigned char *n, unsigned char *a, unsigned abytes, unsigned char *p, unsigned pbytes) 
{
    ocb_crypt(c, k, n, a, abytes, p, pbytes, OCB_ENCRYPT);
}

int ocb_decrypt(unsigned char *p, unsigned char *k, unsigned char *n, unsigned char *a, unsigned abytes, unsigned char *c, unsigned cbytes)
{
    return ocb_crypt(p, k, n, a, abytes, c, cbytes, OCB_DECRYPT);
}


int crypto_aead_encrypt(unsigned char *c, unsigned long long clen, unsigned char *m, unsigned long long mlen, unsigned char *ad, unsigned long long adlen, unsigned char *nsec, unsigned char *npub, unsigned char *k)
{
    clen = mlen + TAGBYTES;
    ocb_crypt(c, (unsigned char *)k, (unsigned char *)npub, (unsigned char *)ad, adlen, (unsigned char *)m, mlen, OCB_ENCRYPT);
    return 0;
}

int crypto_aead_decrypt(unsigned char *m, unsigned long long mlen, unsigned char *nsec, unsigned char *c, unsigned long long clen, unsigned char *ad, unsigned long long adlen, unsigned char *npub, unsigned char *k)
{
    mlen = clen - TAGBYTES;
    return ocb_crypt(m, (unsigned char *)k, (unsigned char *)npub, (unsigned char *)ad, adlen, (unsigned char *)c, clen, OCB_DECRYPT);
}
